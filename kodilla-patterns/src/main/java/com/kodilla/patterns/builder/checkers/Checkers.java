    ...
    public static class CheckersBuilder {
        ...
        public CheckersBuilder figure(String figureType, String figureColour, int x, int y) {
            if (x > Board.MAX_INDEX || x < Board.MIN_INDEX || y > Board.MAX_INDEX || y < Board.MIN_INDEX) {
                throw new IllegalStateException("X and Y should be in range betwen " + Board.MIN_INDEX + " and " + Board.MAX_INDEX + ".");
            }
        ...
        public Checkers build() {
            ...
            if (!(black & white)) {
                throw new IllegalStateException("There are no figures of both colours on the board.");
            }
